...
Conociendo $m$ pares $(ai,bi)$ no se puede determinar el valor $s$.
...
\begin{\begin{enumerate}
  ...
  Se emite como clave pública una colección de valores ($ai$, $bi$).

  \item Cifrado

  Para cada bit $x$ a cifrar se elige una de las parejas y se realiza la siguiente operación:

  ($c1$, $c2$) = ($suma(ai[j])$, $suma(bi[j]) + x * q/2$) para $j€(1, m)$
...
En el apéndice \ref{apendix:test_lwe.py} puede verse un ejemplo de implementación unidimensional del algoritmo.

https://semidoc.github.io/zijlstra-lwe
https://medium.com/asecuritysite-when-bob-met-alice/public-key-encryption-with-learning-with-errors-lwe-1aa6dd1df4e
